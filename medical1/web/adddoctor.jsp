   ...
   <head>
    ...
    <body>
        ...
        <%
            ...
            String daddr=request.getParameter("da");
            String dcity=request.getParameter("dc");
            String dchamber=request.getParameter("dchm");
            String dspecialization=request.getParameter("dspcl");
            String ddegree=request.getParameter("ddeg");
            String dcontact=request.getParameter("dcn");
            String dmail=request.getParameter("dm");
            try
            {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/medical", "root", "subham23");
            String ins="insert into doctor(dname,daddr,dcity,dchamber,dspecialization,ddegree,dcontact,dmail) values(?,?,?,?,?,?,?,?)";
            PreparedStatement p=con.prepareStatement(ins);
            p.setString(1,dname);
            p.setString(2,daddr);
            p.setString(3,dcity);
            p.setString(4,dchamber);
            p.setString(5,dspecialization);
            p.setString(6,ddegree);
            long cont=Long.parseLong(dcontact);
            p.setLong(7,cont);
            p.setString(8,dmail);
